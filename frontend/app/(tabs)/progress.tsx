  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView contentContainerClassName="px-6 pb-8" showsVerticalScrollIndicator={false}>
        <Text className="font-extrabold text-[26px] text-ink tracking-tight pt-2">진척도</Text>
        ...
        <View className="mt-4 flex-row bg-surface rounded-[13px] p-1">
          {["주", "월", "년"].map((p, i) => (
            ...
            >
              <Text className={`font-semibold text-[13px] ${i === 0 ? "text-ink" : "text-muted"}`}>{p}</Text>
            </View>
          ))}
        </View>
        ...
        <View className="mt-[18px]">
          <Text className="text-[13px] text-muted">총 볼륨 · 이번 주</Text>
          <View className="flex-row items-baseline gap-2.5 mt-1">
            <Text className="font-semibold text-[40px] text-ink tracking-tight">
              12,400<Text className="text-[18px] text-muted">kg</Text>
            </Text>
            <Text className="font-semibold text-[13px] text-signal-deep bg-signal-tint px-2 py-0.5 rounded-[7px]">
              ▲ 12%
            </Text>
          </View>
        </View>
        ...
        <View className="mt-[22px]">
          ...
          <View className="flex-row justify-between px-1 pt-2">
            {["월", "화", "수", "목", "금", "토", "일"].map((d, i) => (
              <Text key={d} className={`text-[11px] ${i === 3 ? "text-signal-deep" : "text-muted"}`}>
                {d}
              </Text>
            ))}
          </View>
        ...
        {/* PR */}
        <Text className="font-bold text-[13px] text-ink mt-[22px]">최근 개인 기록</Text>
        <View className="mt-3 gap-2.5">
          {prs.map((pr) => (
            <View key={pr.name} className="flex-row items-center gap-3">
              ...
              <Text className="flex-1 font-semibold text-[15px] text-ink">{pr.name}</Text>
